...
const filter = {
  ...
  [FilterType.TODAY]: (tasks) =>
    tasks.filter(
      (task) => isTaskExpiringToday(task.dueDate) && !task.isArchive
    ),
  [FilterType.FAVORITES]: (tasks) =>
    tasks.filter((task) => task.isFavorite && !task.isArchive),
  [FilterType.REPEATING]: (tasks) =>
    tasks.filter((task) => isTaskRepeating(task.repeating) && !task.isArchive),
